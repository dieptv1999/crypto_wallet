...
import 'package:crypto_wallet/ui/widget/user_card.dart';
...
class ProfileScreenState extends State<ProfileScreen> with TickerProviderStateMixin {
    ...
    singleCardFinish = AnimationController(
      ...
      setState(() {
        scrollPercent = lerpDouble(
            singleCardStart, singleCardEnd, singleCardFinish?.value ?? 0)!;
      });
    });
  ...
  void _hDragUpdate(DragUpdateDetails details) {
    final cardDrag = (details.globalPosition.dx - (startDrag?.dx ?? 0)) / (context.size?.width ?? 0);
...
class Card extends StatelessWidget {
  ...
  Card({required this.imgPath, this.isFollow = false});
